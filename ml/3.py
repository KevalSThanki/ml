...
def id3(tree):
    ...
    for branch, data in tree.items():
        ...
        for i in range(0, len(data.columns)-1):
            ...
            # Sort the rows w.r.t o/p
            sorted_rows = get_sortedn(total_entropy_for_data, sorted_rows, entropy_by_attribute)
...
def get_prediction(tree, data):
    for col in data.keys():
        if isinstance(tree, dict):
            if col in tree.keys():
                get_prediction(tree[col][data[col]], data)
        else:
            print(tree)
            break

get_prediction(root, data.iloc[4,])
